...
def dashboard():
    ...
    if role == 'patient':
        ...
        return render_template('desk_dashboard.html', user=session, doctors=doctors, appointments=appts, alerts=alerts)
...
def ai_verify_document():
    ...
    if not doc or doc.patient_id != session['user_id'] or not get_authorized_appointment(doc.appointment_id, 'patient'):
...
def ai_summary(appt_id):
    ...
    docs = Document.query.filter_by(appointment_id=appt_id, is_verified=True).all()
    if not analysis:
